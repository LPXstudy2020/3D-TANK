...
bool GameObject::isCollision(GameObject * obj)
{
...
void GameObject::solveCollision(GameObject * obj,float dt)
{
}
...
void GameObject::ResizeBuffer(ID3D11Device * device, size_t count)
{
	...
	// 创建实例缓冲区
	device->CreateBuffer(&vbd, nullptr, m_pInstancedBuffer.ReleaseAndGetAddressOf());
...
void GameObject::setHp(int d)
{
	hp = d;
}

void GameObject::increaseHp(int d)
{
	hp += d;
}

void GameObject::decreaseHp(int d)
{
	hp -= d;
}
...
void GameObject::DrawInstanced(ID3D11DeviceContext* deviceContext, BasicEffect& effect, const std::vector<Transform>& data)
{
	...
	deviceContext->Map(m_pInstancedBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedData);
